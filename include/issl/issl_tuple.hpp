...
namespace issl::internal {
    ...
    constexpr auto find_matching_element(const Head &_head,
                                         const Tail &..._tail) {
        return (_head | ... | _tail);
    };

    template <int N, class... Ts, int... _is>
    constexpr auto wrap_elements(Ts... _ts,
...
namespace issl {
    ...
    consteval auto operator+(const Positions<_last, _is...> &,
        ...
        if constexpr (_last % alignof(T) == 0) {
            ...
            constexpr auto last_corrected =
                _last + alignof(T) - (_last % alignof(T));
    ...
    public:
        ...
        Tuple(Types &&..._t) noexcept {
            move_from_types(std::move(_t)..., seq_);
        }
        ...
        constexpr const auto &get() const {
            using Type = internal::nth_element_t<_index, Types...>;
            return *internal::ptr_cast<const Type *>(data_.data() +
                                                     pos<_index>());
        }
        ...
        bool operator==(const Tuple<OtherTypes...> &_other) const noexcept {
            ...
            return [&]<int... _is>(std::integer_sequence<int, _is...>) {
                return (true && ... &&
                        is_same(std::integral_constant<int, _is>{}));
    ...
    private:
        ...
        void copy_from_other(const Tuple<Types...> &_other,
                             std::integer_sequence<int, _is...>) {
            const auto copy_one = [this]<int _i>(
                                      const auto &_other,
                                      std::integral_constant<int, _i>) {
        ...
        void move_from_other(Tuple<Types...> &&_other,
                             std::integer_sequence<int, _is...>) {
            const auto move_one =
                [this]<int _i>(auto &&_other, std::integral_constant<int, _i>) {
                    using Type = internal::nth_element_t<_i, Types...>;
                    ::new (static_cast<void *>(data_.data() + pos<_i>()))
        ...
        template <int... _is>
        void move_from_types(Types &&..._types,
                             std::integer_sequence<int, _is...>) {
            const auto move_one =
                [this]<int _i>(auto &&_t, std::integral_constant<int, _i>) {
                    using Type = internal::nth_element_t<_i, Types...>;
                    ::new (static_cast<void *>(data_.data() + pos<_i>()))
                        Type(std::move(_t));
                };
